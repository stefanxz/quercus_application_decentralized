...
uint8_t security_check(bool* should_check) {
    ...
    *should_check = 0;
    this.tub.passed_security = 1;
    if(get_rfid_data(PAYLOAD)) {
        if(get_rfid_data(PLANE_OR_DROPOFF)){
            this.tub.destination_id = this.nearest[DROPOFF];
            this.tub.destination_type = DROPOFF;
            return DROPOFF;
        }
        if(check_plane_arrived(get_rfid_data(PLANE_ID))){
            this.tub.destination_id = this.id_lookup[this.plane_to_id[PLANE_ID]];
            this.tub.destination_type = PLANE;
            return PLANE;
        }
        this.tub.destination_id = this.nearest[STORAGE];
        this.tub.destination_type = STORAGE;
        return STORAGE;
    }
    this.tub.destination_id = this.nearest[QUARANTINE];
    this.tub.destination_type = QUARANTINE;
    return QUARANTINE;
}
...
void reroute_from_security(){
    ...
    request[MSG_TYPE] = this.tub.destination_type;
    request[REQ_DEST_ID] = this.tub.destination_id;
    request[REQ_DEST_TYPE] = this.tub.destination_type;
    Direction to = this.dir_lookup[this.tub.destination_id];
